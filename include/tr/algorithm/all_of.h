...
#include <tr/algorithm/fwd/all_of.h>
...
template <typename T>
struct all_of_impl<T, std::enable_if_t<is_implemented_v<unpack_impl<T>>>> {

    template <typename Tuple, typename Predicate>
    static constexpr auto apply(Tuple &&tuple, Predicate &&predicate) {

        auto andElems = [&](auto &&...elems) {
            return (invoke(static_cast<Predicate &&>(predicate),
                           static_cast<decltype(elems)>(elems)) &&
                    ...);
        };

        return unpack_impl<T>::apply(static_cast<Tuple &&>(tuple), andElems);
    }
};
...
[[nodiscard]] constexpr auto all_of_t::operator()(Tuple &&tuple,
    ...
    return all_of_impl<tuple_t>::apply(static_cast<Tuple &&>(tuple),
