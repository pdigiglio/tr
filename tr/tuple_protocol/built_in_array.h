...
struct tup_size<T[N]> : std::integral_constant<std::size_t, N> {};

template <typename T, std::size_t N>
struct tup_size<T const[N]> : std::integral_constant<std::size_t, N> {};

namespace detail {
template <std::size_t I, typename T>
constexpr decltype(auto) array_get_impl(T &&arr) noexcept {
    using array_t = std::remove_reference_t<T>;
    static_assert(std::is_array_v<array_t>,
                  "T is not a reference to a built-in array");
